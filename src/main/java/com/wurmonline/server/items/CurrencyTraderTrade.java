...
public class CurrencyTraderTrade extends Trade {
    private static final Logger logger = Logger.getLogger(CurrencyTraderTrade.class.getName());
    ...
    public CurrencyTraderTrade(Creature player, Creature trader) {
        ...
        creatureTwoOfferWindow = new CurrencyTradingWindow(trader, player, true, 1L, this);
        creatureOneOfferWindow = new CurrencyTradingWindow(player, trader, true, 2L, this);
        creatureOneRequestWindow = new CurrencyTradingWindow(trader, player, false, 3L, this);
        creatureTwoRequestWindow = new CurrencyTradingWindow(player, trader, false, 4L, this);
    }
            ...
            if ((!this.creatureTwo.isPlayer() || this.creatureTwo.hasLink()) && !this.creatureTwo.isDead()) {
                if (this.creatureOneRequestWindow.hasInventorySpace() && this.creatureTwoRequestWindow.hasInventorySpace()) {
                    ...
                    if (ok) {
                        ...
                        if (CurrencyTraderTemplate.isCurrencyTrader(creatureTwo)) {
