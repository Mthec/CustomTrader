...
public class StatTradingWindow extends TradingWindow {
    ...
    private final StateTraderTrade trade;
    private final Stat stat;

    StatTradingWindow(Creature owner, Creature watcher, boolean offer, long wurmId, StateTraderTrade trade) {
    ...
    private static Logger getLogger(long wurmId) {
        String name = "currencytrader" + wurmId;
            ...
            if (this.items != null) {
                ...
                for (Item tit : this.items) {
                    ...
                    for (Item lAllItem : allItems) {
                        ...
                        paymentRequired += lAllItem.getPrice();
                    }
                }
    ...
    void swapOwners() {
        if (!offer) {
            ...
            Shop shop;
            if (CurrencyTraderTemplate.isCurrencyTrader(windowOwner)) {
            ...
            if (!problems) {
                ...
                if (StatTraderTemplate.is(watcher)) {
                    shop.addMoneyEarned(statCounter);
                }
            } else {
